...
const eyeOpen = `<svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
...
let is_open = false, selectedPic = null;
...
const eyeBtn = $(".password__eye");
const msgError = $("#msgError");
const picError = $("#picError");
const usernameChars = $(".usernameChars");
const loader = $(".loader");
const pages = $(".form_pages");
const nextBtn = $(".form__next");
...
function charsLeft(){
    const chars = username.value.length;
    usernameChars.textContent = 16 - chars;
    if(chars > 16){
        usernameChars.classList.add("invalidChars");
    }else{
        usernameChars.classList.remove("invalidChars");
    }
}
...
function showPicError(text){
    picError.textContent = text;
    setTimeout(()=>{
...
form.addEventListener("submit", async(evt)=>{
    ...
    if(!selectedPic){
        showPicError("No has seleccionado ninguna foto");
        return;
    }
    ...
    const isPasswdValid = passwordRegex.test(password.value);
    
    if(!isEmailValid){
        goBack();
        showError("Formato del email incorrecto");
        return;
    }

    if(!isUsernameValid){
        goBack();
        showError("Formato del usuario incorrecto");
        return;
    }

    if(!isPasswdValid){
        goBack();
        showError("Mínimo 6 caracteres para la contraseña");
        return;
    }
